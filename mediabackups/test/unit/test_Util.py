...
class Test_Util(unittest.TestCase):
    ...
    def test_sha1sum(self):
        ...
        with patch('builtins.open', mock_open(read_data=data)):
            self.assertEqual(sha1sum, Util.sha1sum(None))
        # non-trivial size file
        data = self.example_file
    ...
    def test_sha256sum(self):
        ...
        with patch('builtins.open', mock_open(read_data=data)):
            self.assertEqual(sha256sum, Util.sha256sum(None))
        # checksum with leading zeros (we zerofill)
        data = b'hello28'
        sha256sum = '02a13c40ba00dc0fb199d3cbe5b01be59d937775890243fd411bdf001935ffc8'
        with patch('builtins.open', mock_open(read_data=data)):
            self.assertEqual(sha256sum, Util.sha256sum(None))
        # non-trivial size file
        data = self.example_file
        sha256sum = '96397d8e7962f523bb8a76dbd60c757990bd78d6785673946e237fd31fd1c7c9'
        with patch('builtins.open', mock_open(read_data=data)):
            self.assertEqual(sha256sum, Util.sha256sum(None))
